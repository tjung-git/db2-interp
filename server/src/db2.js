...
try {
  ...
  /* optional dependency not installed; stay in mock mode */
}

export async function fetchSeriesFromDb2({ table, xcol, ycol, limit = 200 }) {
  if (!process.env.DB2_CONN_STR) {
    // "Looks connected" but stays local; caller can still proceed with manual data.
